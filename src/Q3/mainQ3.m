...
for i = 1:iteration
   samples = poissonSamples(lambda,NSet(1));
...
for i = 1:iteration
    samples = poissonSamples(lambda,NSet(2));
...
for i = 1:iteration
    samples = poissonSamples(lambda,NSet(3));
...
xlabel('Sample Mean'); ylabel('Frequency of Sample Mean');
set(gca,'FontSize',14,'FontWeight','bold')
print('-djpeg', ['Histogram_N_',num2str(NSet(3)),'.jpg'], '-r300');
close all;


for i = 1:iteration
    samples = poissonSamples(lambda,NSet(4));
...
xlabel('Sample Mean'); ylabel('Frequency of Sample Mean');
set(gca,'FontSize',14,'FontWeight','bold')
print('-djpeg', ['Histogram_N_',num2str(NSet(4)),'.jpg'], '-r300');
close all;
...
% tabulate(meansforN10)

pmf = poissonpmf(samples,lambda);
scatter(samples,pmf,'filled')
 title(['PMF of Poission Samples for lambda = 10']);
xlabel('k'); ylabel('P(X = k)');
set(gca,'FontSize',14,'FontWeight','bold')
print('-djpeg', ['PMF_Poission.jpg'], '-r300');
close all;
